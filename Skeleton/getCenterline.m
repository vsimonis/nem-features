function list = getCenterline( segment, endpoint, branchpoint,posture, estLength  )
   ...
   elseif posture == 2 % Delta Loop
    ...
    elseif posture == 4 || posture == 3 || posture == 0
        ...
        for k = 1:length(segment)
            ...
            %Segment 1 is associated with endpoint 1
            if ~isempty(find(ismember(endpoint1,pixelList,'rows'),1))
                segment1 = pixelList;
                usedSegments = [usedSegments,k];

             %Segment2 is associated with endpoint 2
            elseif  ~isempty(find(ismember(endpoint2,pixelList,'rows'),1))
                segment2 = pixelList;
                usedSegments = [usedSegments,k];
            else
                %do nothing
        ...
        %Identifies whether the topology of a looping
        % nematode skeleton  consists of one loop with two branche off of
        % it (configuration one), or two branches connected to one branch, 
        % which is connected to the loop (configuration 2), or two branches 
        % connectd to one location on the loop (configuration 3).  The three
        % topologies represent the allowable topologies for a nematode in a 
        % loop with two ends visible 

        if size(branchpoint,1) == 1
         %both end segments connect to the loop through the same
         %branchpoint
         config = 3;
        else

            %Determine whether segment 1 and 2
        ...
        if config == 2 %fish shape, one branch connects to the loop
            ...
            for k = 1:length(segment)
                if ~ismember(k, usedSegments)
                    ...
                    % segment3
                    if    ( ~isempty(find(ismember(branchpoint(1,:),pixelList,'rows'),1 )) )...
                           && ( ~isempty(find(ismember(branchpoint(2,:),pixelList,'rows'),1)) )
        ...
        if config == 3
            ...
            for k = 1:length(segment)
                ...
                if ~ismember(k, usedSegments)
                    ...
                    loop  = segment(k).PixelList;
                end
            end
            
            %correctly orient the segment 1 to be the first segment, 
            % so the first pixel is the endpoint1
            if ~isequal( segment1(1,:), endpoint1)
                segment1 = reverserows(segment1);
            end
            
            %correctly orient segment 2 to be the last segment,
            % so the first pixel is the branchpoint
            if ~isequal(segment2(1,:), branchpoint(1,:));
                   segment2 = reverserows(segment2);
            end
             
            %Re-orient the loop so that it connects segments 1 and 2
            %correctly.  Use end of segment 2 and vector connecting segment 1
            %to the branchpoint to determien correct orientation.  
             ...
             position = sign( (B(1)-A(1))*(M(2)-A(2)) - (B(2)-A(2))*(M(1)-A(1)) );
             
             if position > 0 && posture == 3
                 loop = reverserows(loop);
             elseif position < 0 && posture == 4
                 loop = reverserows(loop);
             else
             ...
             list = [segment1(1:end-1,:); loop; segment2(2:end,:)];
